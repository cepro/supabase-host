-- Deploy supabase_setup:0004_post_setup to pg
--
-- This migration performs final Supabase setup including dashboard user creation and
-- extension event triggers for use with external PostgreSQL (specifically TimescaleDB).
-- Modifications have been made from the standard Supabase setup to work within managed PostgreSQL constraints:
--
-- MODIFICATIONS FOR TIMESCALEDB/MANAGED POSTGRESQL:
-- 1. Commented out postgres role modification - conflicts with TimescaleDB's tsdbexplorer role
-- 2. Commented out pg_cron functionality - not available on TimescaleDB Cloud
-- 3. Commented out pg_net functionality - not available on TimescaleDB Cloud
-- 4. Removed REPLICATION privilege from dashboard_user - not available in managed services
-- 5. Changed database name from postgres to tsdb for dashboard_user grants
...
-- MODIFIED: Commented out postgres role modification - conflicts with TimescaleDB's tsdbexplorer role
...
-- MODIFIED: Removed REPLICATION privilege - not available in managed TimescaleDB
CREATE ROLE dashboard_user NOSUPERUSER CREATEDB CREATEROLE;
-- MODIFIED: Changed database name from postgres to tsdb
