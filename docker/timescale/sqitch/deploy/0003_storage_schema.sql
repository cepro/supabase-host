-- Deploy supabase_setup:0003_storage_schema to pg
--
-- This migration creates the Supabase storage schema and supabase_storage_admin role
-- for use with external PostgreSQL (specifically TimescaleDB). Modifications have been made
-- from the standard Supabase setup to work within managed PostgreSQL constraints:
--
-- MODIFICATIONS FOR TIMESCALEDB/MANAGED POSTGRESQL:
-- 1. Removed 'postgres' role from grant statements - conflicts with TimescaleDB's tsdbexplorer role
-- 2. Added GRANT supabase_storage_admin TO tsdbadmin - required for ownership changes during migration
...
-- MODIFIED: Removed 'postgres' role - conflicts with TimescaleDB's tsdbexplorer role
...
-- ADDED: Grant role to tsdbadmin so ownership can be changed during migration
GRANT supabase_storage_admin TO tsdbadmin;
